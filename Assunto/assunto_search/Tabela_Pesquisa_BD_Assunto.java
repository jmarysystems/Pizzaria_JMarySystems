 ...
 * @author ana
 */
public class Tabela_Pesquisa_BD_Assunto extends javax.swing.JPanel {
   
    private Assunto_Home                 Assunto_Home;
    private Menu_Pesquisa_Assunto        Menu_Pesquisa_Assunto;
        
    private List<assunto.Assunto> list;
    private ListSelectionModel                      lsmPesquisa;
    private final DefaultTableModel                 tmPesquisa = new DefaultTableModel( null, new String[]{ "  ID","Assunto"} );
    
    public Tabela_Pesquisa_BD_Assunto( Assunto_Home Assunto_Home2 ) {
    ...
    public void tabela(Menu_Pesquisa_Assunto Menu_Pesquisa_Assunto2){ 
                
        try {                
            Menu_Pesquisa_Assunto = Menu_Pesquisa_Assunto2;                                    
        } catch( Exception e ){ JOPM JOptionPaneMod = new JOPM( 2, "tabela, \n"
                + e.getMessage() + "\n", "Tabela_Pesquisa_BD_Assunto" ); }     
    }
    ...
    private void tabelaInicio(){
        new Thread() {   @Override public void run() { try { Thread.sleep( 1 ); 
            ...
            mostrarLista( list );
                ...
                + e.getMessage() + "\n", "Tabela_Pesquisa_BD_Assunto" ); } } }.start();              
    }
    ...
    public void pesquisa(Query q2){
        ...
        } catch( Exception e ){ while ( tmPesquisa.getRowCount() > 0){ tmPesquisa.removeRow(0); }
    ...
    public void mostrarLista( List<assunto.Assunto> listR ){  try {                    
            ...
            if ( listR.isEmpty() ){ list.clear(); //JOptionPane.showMessageDialog( null , "Nenhum dado encontrado!"); 
                ...
                for (int i = 0; i < listR.size(); i++){
                    ...
                    tmPesquisa.setValueAt( listR.get(i).getAssunto()       , i, 1);
                }
                list = listR;
            }    
    ...
    private void tbPesquisaLinhaSelecionada() { 
        try{
            if ( tbPesquisa.getSelectedRow() != -1){
                ...
                Menu_Pesquisa_Assunto.IDSELECIONADA = list.get( tbPesquisa.getSelectedRow() ).getId();
                
                Menu_Pesquisa_Assunto.botoesEstado(true, true, true);
                Menu_Pesquisa_Assunto.btAlterar    .setEnabled( true );
                Menu_Pesquisa_Assunto.btExcluir    .setEnabled( true );  
                Menu_Pesquisa_Assunto.btVisualizar .setEnabled( true );  
            } else{
                      
                Menu_Pesquisa_Assunto.IDSELECIONADA = -1;  
                Menu_Pesquisa_Assunto.botoesEstado(false, false, false);
            }
        } catch( Exception e ) {}
    }
